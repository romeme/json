...
import static ru.romeme.json.JSTools.key;

public class ObjectParsingTest {

    @Test
    public void empty() {
        Assert.assertTrue(Json.object("{}").map(Map::isEmpty).orElse(false));
        Assert.assertTrue(Json.object("{ }").map(Map::isEmpty).orElse(false));
        Assert.assertTrue(Json.object(" {}").map(Map::isEmpty).orElse(false));
        Assert.assertTrue(Json.object("{} ").map(Map::isEmpty).orElse(false));
        Assert.assertTrue(Json.object("{\r\n\t\b\f}").map(Map::isEmpty).orElse(false));
        ...
        List<Integer> rs = Json.object("")
                ...
                .stream()
                .flatMap(List::stream)
                .reduce(Optional.of(new ArrayList<Integer>()),
                        (accOP, cn) ->
                                accOP.flatMap(acc ->
                                        JSTools.integerOP(cn)
                                                .map(vv -> new ArrayList<>(acc) {{ add(vv);}})
                                ),
                        JSTools.join()
    ...
    public void singleKey() {
        ...
        Assert.assertTrue(
                Json.object("{\"key\":1}")
                        .map(map -> map.containsKey("key")).orElse(false));
        Assert.assertTrue(
                Json.object("{ \"key\":1}")
